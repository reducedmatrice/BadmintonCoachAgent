...
>
> 当前 1.0 阶段已完成的核心能力：
> - 基于 DeerFlow custom agent 机制跑通 `badminton-coach`
> - `prematch` / `postmatch` 规则链路可用
> - `coach_profile.json`、`memory.json`、`memory/reviews/*.md` 三层 memory 已落地
> - 天气上下文、基础评测脚本、结构化日志与相关测试已接入
>
...
本项目正式命名为 **Badminton-Coach-Agent**。项目基于现有 Agent 基础设施进行垂直场景定制化，目标是落地一个可真实使用、可持续迭代的 **羽毛球教练 + 身体指引 Coach Agent**。
本项目不追求功能大而全，而是采用“低侵入增量开发”方式，只补齐与教练场景直接相关的能力：
...
- 多用户权限与租户隔离
- 商业化计费、运营后台、复杂报表
- 重写 Agent graph 注册方式或新增独立 LangGraph graph
...
| 如果只做最小可讲 demo，建议目标是 **2-3 天完成 A + B + C**。 | <br />                       | <br />  | <br />    |
...
- 增加伤病恢复期专用模式
- 输出周报与月度训练趋势总结
- 从被动问答扩展到“主动式教练闭环”：赛前提醒、赛后催复盘、恢复跟进、周目标回顾
- 增加“训练建议采纳率、复盘完成率、弱项复发率”这类产品效果指标，验证 Agent 是否真的提升训练质量

### 7.2 Agent 能力扩展

- 从单 Agent 问答升级为“条件式多 Agent 协作”，按场景拆分 `router / technique analyst / recovery analyst / memory curator`
- 保持主链路默认单 Agent，仅在长文总结、周报、复杂恢复分析等非实时场景按需启用 subagent
- 为不同 Agent 定义明确输入输出契约，避免多 Agent 串联后出现职责重叠和结果冲突
- 增加“建议溯源”能力，让最终输出能显式标注来自 `coach_profile`、日期日志、外部上下文还是模型推断
...
- 让评估从“输出质量”继续扩展到“真实使用效果”，包括提醒触达率、建议采纳率、长期留存和训练完成率

### 7.4 记忆与画像扩展

- 将 `coach_profile.json` 从静态画像升级为“趋势化运动画像”，支持弱项活跃度、时间衰减、阶段目标切换
- 为结构化弱项和恢复状态补充 evidence chain，保留“结论来自哪几次训练/哪条截图/哪次复盘”
- 增加冲突消解机制，避免一次偶发状态覆盖长期稳定结论
- 将日期日志从简单追加升级为可摘要、可检索、可生成阶段总结的长期训练时间线
...
- 后续应把多模态真正并入飞书主入口，而不是长期停留在“Web 图片验证、IM 文本主流程”的割裂形态
