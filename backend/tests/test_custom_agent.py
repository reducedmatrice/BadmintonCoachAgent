...
class TestLoadAgentSoul:
    ...
    def test_badminton_coach_soul_and_config_load(self, tmp_path):
        expected_soul = "You are a badminton coach."
        _write_agent(
            tmp_path,
            "badminton-coach",
            {
                "name": "badminton-coach",
                "description": "Badminton coach focused on training and recovery",
            },
            soul=expected_soul,
        )
        ...
        with patch("deerflow.config.agents_config.get_paths", return_value=_make_paths(tmp_path)):
            ...
            cfg = load_agent_config("badminton-coach")
            soul = load_agent_soul("badminton-coach")

        assert cfg is not None
        assert cfg.name == "badminton-coach"
        assert cfg.description == "Badminton coach focused on training and recovery"
        assert soul == expected_soul
...
class TestMemoryFilePath:
    ...
    def test_badminton_coach_memory_data_is_isolated(self, tmp_path):
        import deerflow.agents.memory.updater as updater_mod
        from deerflow.config.memory_config import MemoryConfig
        ...
        memory_data = {
            "version": "1.0",
            "facts": [{"content": "Left knee soreness after doubles match."}],
        }
        (agent_dir / "memory.json").write_text(json.dumps(memory_data), encoding="utf-8")

        with (
            patch("deerflow.agents.memory.updater.get_paths", return_value=_make_paths(tmp_path)),
            patch("deerflow.agents.memory.updater.get_memory_config", return_value=MemoryConfig(storage_path="")),
            patch.dict("deerflow.agents.memory.updater._memory_cache", {}, clear=True),
        ):
            loaded = updater_mod.get_memory_data("badminton-coach")

        assert loaded["facts"] == [{"content": "Left knee soreness after doubles match."}]
